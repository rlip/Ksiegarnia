...
class CommonFunction implements ServiceLocatorAwareInterface
{
    ...
    {
        if (self::DEBUG) {
            echo $this->_getExecutionTime() . ' ' . round(memory_get_usage() / 1024 / 1024) . 'MB ' . $this->cleanChars($sMessage). "\n";
        }
    }

    public function getConfigArray(){
        if($this->_aConfig === null){
            $aData = $this->getEntityManager()
                ->getRepository('Application\Entity\EntityConfig')
                ...
                foreach($aData as $aRow){
                    $sKey = $aRow['configKey'];
                    $this->_aConfig[$sKey] = $aRow['configValue'];
                }
        }
    ...
    public function getConfigParam($sParam){
        $aConfig = $this->getConfigArray();
        if(is_array($aConfig) and isset($aConfig[$sParam])){
    ...
    {
        return '[' . (time() - $this->_iTime) . 's]';
    }

    public function cleanChars($str, $utf8 = true)
    {
        $str = (string)$str;
        if (is_null($utf8)) {
        ...
        $transliteration = array(
            ...
            'ჯ' => 'j', 'ჰ' => 'h', 'ʼ' => "'", '̧' => '', 'ḩ' => 'h', '‘' => "'",
            '’' => "'", 'ừ' => 'u', 'ế' => 'e', 'ả' => 'a', 'ị' => 'i', 'ậ' => 'a', 'ệ' => 'e',
            'ỉ' => 'i', 'ộ' => 'o', 'ồ' => 'o', 'ề' => 'e', 'ơ' => 'o', 'ạ' => 'a', 'ẵ' => 'a',
            'ư' => 'u', 'ắ' => 'a', 'ằ' => 'a', 'ầ' => 'a', 'ḑ' => 'd', 'Ḩ' => 'H', 'Ḑ' => 'D',
            'ş' => 's', 'ţ' => 't'
        );
