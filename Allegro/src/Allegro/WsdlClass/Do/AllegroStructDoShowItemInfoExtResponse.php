<?php
/**
 * File for class AllegroStructDoShowItemInfoExtResponse
 * @package Allegro
 * @subpackage Structs
 * @date 2015-04-14
 * @author Roman Lip
 * @version 1
 */
/**
 * This class stands for AllegroStructDoShowItemInfoExtResponse originally named doShowItemInfoExtResponse
 * Meta informations extracted from the WSDL
 * - from schema : {@link https://webapi.allegro.pl/service.php?wsdl}
 * @package Allegro
 * @subpackage Structs
 * @date 2015-04-14
 * @author Roman Lip
 * @version 1
 */
class AllegroStructDoShowItemInfoExtResponse extends AllegroWsdlClass
{
    /**
     * The itemListInfoExt
     * @var AllegroStructItemInfoExt
     */
    public $itemListInfoExt;
    /**
     * The itemCatPath
     * Meta informations extracted from the WSDL
     * - minOccurs : 0
     * @var AllegroStructArrayOfItemcatlist
     */
    public $itemCatPath;
    /**
     * The itemImgList
     * Meta informations extracted from the WSDL
     * - minOccurs : 0
     * @var AllegroStructArrayOfItemimagelist
     */
    public $itemImgList;
    /**
     * The itemAttribList
     * Meta informations extracted from the WSDL
     * - minOccurs : 0
     * @var AllegroStructArrayOfAttribstruct
     */
    public $itemAttribList;
    /**
     * The itemPostageOptions
     * Meta informations extracted from the WSDL
     * - minOccurs : 0
     * @var AllegroStructArrayOfPostagestruct
     */
    public $itemPostageOptions;
    /**
     * The itemPaymentOptions
     * @var AllegroStructItemPaymentOptions
     */
    public $itemPaymentOptions;
    /**
     * The itemCompanyInfo
     * @var AllegroStructCompanyInfoStruct
     */
    public $itemCompanyInfo;
    /**
     * The itemProductInfo
     * @var AllegroStructProductStruct
     */
    public $itemProductInfo;
    /**
     * The itemVariants
     * Meta informations extracted from the WSDL
     * - minOccurs : 0
     * @var AllegroStructArrayOfItemvariantstruct
     */
    public $itemVariants;
    /**
     * Constructor method for doShowItemInfoExtResponse
     * @see parent::__construct()
     * @param AllegroStructItemInfoExt $_itemListInfoExt
     * @param AllegroStructArrayOfItemcatlist $_itemCatPath
     * @param AllegroStructArrayOfItemimagelist $_itemImgList
     * @param AllegroStructArrayOfAttribstruct $_itemAttribList
     * @param AllegroStructArrayOfPostagestruct $_itemPostageOptions
     * @param AllegroStructItemPaymentOptions $_itemPaymentOptions
     * @param AllegroStructCompanyInfoStruct $_itemCompanyInfo
     * @param AllegroStructProductStruct $_itemProductInfo
     * @param AllegroStructArrayOfItemvariantstruct $_itemVariants
     * @return AllegroStructDoShowItemInfoExtResponse
     */
    public function __construct($_itemListInfoExt = NULL,$_itemCatPath = NULL,$_itemImgList = NULL,$_itemAttribList = NULL,$_itemPostageOptions = NULL,$_itemPaymentOptions = NULL,$_itemCompanyInfo = NULL,$_itemProductInfo = NULL,$_itemVariants = NULL)
    {
        parent::__construct(array('itemListInfoExt'=>$_itemListInfoExt,'itemCatPath'=>($_itemCatPath instanceof AllegroStructArrayOfItemcatlist)?$_itemCatPath:new AllegroStructArrayOfItemcatlist($_itemCatPath),'itemImgList'=>($_itemImgList instanceof AllegroStructArrayOfItemimagelist)?$_itemImgList:new AllegroStructArrayOfItemimagelist($_itemImgList),'itemAttribList'=>($_itemAttribList instanceof AllegroStructArrayOfAttribstruct)?$_itemAttribList:new AllegroStructArrayOfAttribstruct($_itemAttribList),'itemPostageOptions'=>($_itemPostageOptions instanceof AllegroStructArrayOfPostagestruct)?$_itemPostageOptions:new AllegroStructArrayOfPostagestruct($_itemPostageOptions),'itemPaymentOptions'=>$_itemPaymentOptions,'itemCompanyInfo'=>$_itemCompanyInfo,'itemProductInfo'=>$_itemProductInfo,'itemVariants'=>($_itemVariants instanceof AllegroStructArrayOfItemvariantstruct)?$_itemVariants:new AllegroStructArrayOfItemvariantstruct($_itemVariants)),false);
    }
    /**
     * Get itemListInfoExt value
     * @return AllegroStructItemInfoExt|null
     */
    public function getItemListInfoExt()
    {
        return $this->itemListInfoExt;
    }
    /**
     * Set itemListInfoExt value
     * @param AllegroStructItemInfoExt $_itemListInfoExt the itemListInfoExt
     * @return AllegroStructItemInfoExt
     */
    public function setItemListInfoExt($_itemListInfoExt)
    {
        return ($this->itemListInfoExt = $_itemListInfoExt);
    }
    /**
     * Get itemCatPath value
     * @return AllegroStructArrayOfItemcatlist|null
     */
    public function getItemCatPath()
    {
        return $this->itemCatPath;
    }
    /**
     * Set itemCatPath value
     * @param AllegroStructArrayOfItemcatlist $_itemCatPath the itemCatPath
     * @return AllegroStructArrayOfItemcatlist
     */
    public function setItemCatPath($_itemCatPath)
    {
        return ($this->itemCatPath = $_itemCatPath);
    }
    /**
     * Get itemImgList value
     * @return AllegroStructArrayOfItemimagelist|null
     */
    public function getItemImgList()
    {
        return $this->itemImgList;
    }
    /**
     * Set itemImgList value
     * @param AllegroStructArrayOfItemimagelist $_itemImgList the itemImgList
     * @return AllegroStructArrayOfItemimagelist
     */
    public function setItemImgList($_itemImgList)
    {
        return ($this->itemImgList = $_itemImgList);
    }
    /**
     * Get itemAttribList value
     * @return AllegroStructArrayOfAttribstruct|null
     */
    public function getItemAttribList()
    {
        return $this->itemAttribList;
    }
    /**
     * Set itemAttribList value
     * @param AllegroStructArrayOfAttribstruct $_itemAttribList the itemAttribList
     * @return AllegroStructArrayOfAttribstruct
     */
    public function setItemAttribList($_itemAttribList)
    {
        return ($this->itemAttribList = $_itemAttribList);
    }
    /**
     * Get itemPostageOptions value
     * @return AllegroStructArrayOfPostagestruct|null
     */
    public function getItemPostageOptions()
    {
        return $this->itemPostageOptions;
    }
    /**
     * Set itemPostageOptions value
     * @param AllegroStructArrayOfPostagestruct $_itemPostageOptions the itemPostageOptions
     * @return AllegroStructArrayOfPostagestruct
     */
    public function setItemPostageOptions($_itemPostageOptions)
    {
        return ($this->itemPostageOptions = $_itemPostageOptions);
    }
    /**
     * Get itemPaymentOptions value
     * @return AllegroStructItemPaymentOptions|null
     */
    public function getItemPaymentOptions()
    {
        return $this->itemPaymentOptions;
    }
    /**
     * Set itemPaymentOptions value
     * @param AllegroStructItemPaymentOptions $_itemPaymentOptions the itemPaymentOptions
     * @return AllegroStructItemPaymentOptions
     */
    public function setItemPaymentOptions($_itemPaymentOptions)
    {
        return ($this->itemPaymentOptions = $_itemPaymentOptions);
    }
    /**
     * Get itemCompanyInfo value
     * @return AllegroStructCompanyInfoStruct|null
     */
    public function getItemCompanyInfo()
    {
        return $this->itemCompanyInfo;
    }
    /**
     * Set itemCompanyInfo value
     * @param AllegroStructCompanyInfoStruct $_itemCompanyInfo the itemCompanyInfo
     * @return AllegroStructCompanyInfoStruct
     */
    public function setItemCompanyInfo($_itemCompanyInfo)
    {
        return ($this->itemCompanyInfo = $_itemCompanyInfo);
    }
    /**
     * Get itemProductInfo value
     * @return AllegroStructProductStruct|null
     */
    public function getItemProductInfo()
    {
        return $this->itemProductInfo;
    }
    /**
     * Set itemProductInfo value
     * @param AllegroStructProductStruct $_itemProductInfo the itemProductInfo
     * @return AllegroStructProductStruct
     */
    public function setItemProductInfo($_itemProductInfo)
    {
        return ($this->itemProductInfo = $_itemProductInfo);
    }
    /**
     * Get itemVariants value
     * @return AllegroStructArrayOfItemvariantstruct|null
     */
    public function getItemVariants()
    {
        return $this->itemVariants;
    }
    /**
     * Set itemVariants value
     * @param AllegroStructArrayOfItemvariantstruct $_itemVariants the itemVariants
     * @return AllegroStructArrayOfItemvariantstruct
     */
    public function setItemVariants($_itemVariants)
    {
        return ($this->itemVariants = $_itemVariants);
    }
    /**
     * Method called when an object has been exported with var_export() functions
     * It allows to return an object instantiated with the values
     * @see AllegroWsdlClass::__set_state()
     * @uses AllegroWsdlClass::__set_state()
     * @param array $_array the exported values
     * @return AllegroStructDoShowItemInfoExtResponse
     */
    public static function __set_state(array $_array,$_className = __CLASS__)
    {
        return parent::__set_state($_array,$_className);
    }
    /**
     * Method returning the class name
     * @return string __CLASS__
     */
    public function __toString()
    {
        return __CLASS__;
    }
}
